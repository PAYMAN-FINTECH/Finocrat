...
export class PayInHistoryComponent implements OnInit {
  ...
  // Build receipt HTML with table layout
  receiptDiv.innerHTML = `
    <div style="text-align: center; border-bottom: 3px solid #6a5af9; padding-bottom: 12px; margin-bottom: 20px;">
      <h2 style="margin:0; color:#6a5af9; font-weight:600;">PAYMENT RECEIPT</h2>
      <p style="margin:5px 0 0; color:#666; font-size:13px;">PayIn Transaction</p>
    </div>
    ...
    <table style="width:100%; border-collapse: collapse; margin-bottom: 20px; font-size:14px;">
      <tr>
        ...
        <td style="padding: 8px; border: 1px solid #e0e0e0;">${item.paymentId}</td>
      </tr>
      <tr>
    ...
    <table style="width:100%; border-collapse: collapse; margin-bottom: 20px; font-size:14px;">
      ...
      <tr>
        ...
        <td style="padding: 10px; border: 1px solid #ccc; text-align:right;">${item.payInCommission?.toFixed(2) || '0.00'}</td>
      </tr>
      <tr style="background:#f9f9f9;">
        <td style="padding: 10px; border: 1px solid #ccc;"><strong>Net Total</strong></td>
        <td style="padding: 10px; border: 1px solid #ccc; text-align:right; font-weight:600;">
          ${(item.amount - (item.payInCommission || 0)).toFixed(2)}
        </td>
      </tr>
    </table>
  ...
  try {
    ...
    pdf.save(`Receipt_${item.paymentId}.pdf`);
  } catch (error) {
